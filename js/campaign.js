...
const toggleClass = (element, className, condition) => {
  // 조건에 따라 요소의 클래스를 토글하는 함수
  element.classList.toggle(className, condition);
};

const setBorder = (element, border) => {
  // 요소의 테두리를 설정하는 함수
...
const updateAdObjectType = (e) => {
  // 광고 객체의 타입을 업데이트하는 함수
...
yearTitle.innerHTML =
  yearArr[yearArr.indexOf(currentDate.getFullYear().toString())]; // 현재 연을 연 선택자에 설정
...
const updateCalendarButtons = () => {
  ...
  disableButton(
    calLeftBtn,
    currentMonth <= new Date().getMonth() && currentYear === 0
  ); // 왼쪽 버튼 상태 업데이트
...
const highlightDates = () => {
  ...
  if (startDate) {
    // 시작 날짜가 있는 경우
    const startDayElement = Array.from(days).find(
      (day) => new Date(day.dataset.date).getTime() === startDate.getTime()
    ); // 시작 날짜 요소 찾기
  ...
  if (endDate) {
    // 종료 날짜가 있는 경우
    const endDayElement = Array.from(days).find(
      (day) => new Date(day.dataset.date).getTime() === endDate.getTime()
    ); // 종료 날짜 요소 찾기
    if (endDayElement) addClass(endDayElement, "end-date"); // 종료 날짜 클래스 추가
  }
};

const highlightWeeks = () => {
  // 선택된 주 하이라이트 함수
  ...
  selectedWeeks.forEach((weekString) => {
    // 선택된 주에 대해
    const weekDates = weekString
      .split(",")
      .map((dateString) => new Date(dateString)); // 주 날짜들을 배열로 변환
    weekDates.forEach((date) => {
      // 주의 각 날짜에 대해
      const dayElement = Array.from(days).find(
        (day) => new Date(day.dataset.date).getTime() === date.getTime()
      ); // 해당 날짜 요소 찾기
...
const renderCalendar = (date) => {
  ...
  for (
    let i = prevMonthLastDay.getDate() - prevMonthDaysToShow + 1;
...
addEventListener(calLeftBtn, "click", () => {
  // 왼쪽 버튼 클릭 이벤트
  if (currentMonth === 0) {
    currentMonth = 12;
    currentYear--;
...
addEventListener(calRightBtn, "click", () => {
  // 오른쪽 버튼 클릭 이벤트
  currentMonth++;
...
bannerList.forEach((node, idx) =>
  addEventListener(node, "click", () => updateBannerSelection(idx))
); // 모든 배너에 클릭 이벤트 추가
...
addEventListener(purchaseBtn, "click", () => {
  // 구매 버튼 클릭 이벤트
...
const stopDragging = () => {
  ...
  if (movedBy < -50 && currentIndex < slideCarousel.length - 1)
    currentIndex += 1; // 이동 거리가 -50 이하이고 현재 인덱스가 마지막이 아닌 경우 인덱스 증가
...
const moveSlider = (position) => {
  ...
  if (
    ...
    return; // 첫 번째 항목에서 왼쪽으로 이동하거나 마지막 항목에서 오른쪽으로 이동하는 경우 함수 종료
...
addEventListener(slideList, "touchstart", (e) =>
  startDragging(e.touches[0].clientX)
); // 슬라이드 리스트에 터치스타트 이벤트 추가
addEventListener(slideList, "touchend", stopDragging); // 슬라이드 리스트에 터치엔드 이벤트 추가
addEventListener(slideList, "touchmove", (e) =>
  moveSlider(e.touches[0].clientX)
); // 슬라이드 리스트에 터치무브 이벤트 추가
